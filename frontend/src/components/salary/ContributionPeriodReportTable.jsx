...
const ContributionPeriodReportTable = ({
  ...
  const totals = rows.reduce(
    (acc, row) => ({
      ...
      employer_epf_payment: acc.employer_epf_payment + (Number(row.employer_epf_payment) || 0),
    ...
    <div>
      <div className="mb-6 rounded-xl border border-gray-200 bg-linear-to-r from-gray-50 to-blue-50 px-6 py-5">
        ...
        <div className="mt-4 text-sm text-gray-600 space-y-0.5">
          ...
          {periodLabel && (
            <p>
              <span className="font-medium text-gray-700">Period:</span> {periodLabel}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 overflow-hidden">
        <table className="min-w-full text-sm table-fixed">
          <thead className="bg-linear-to-r from-gray-50 to-blue-50 text-gray-700 uppercase text-xs border-b-2 border-gray-200">
            <tr>
              <th className="px-6 py-4 text-left font-semibold w-1/4">Period</th>
              <th className="px-6 py-4 text-right font-semibold w-1/4">Employee EPF (8%)</th>
              <th className="px-6 py-4 text-right font-semibold w-1/4">Employer EPF (12%)</th>
              <th className="px-6 py-4 text-right font-semibold w-1/4">Employer ETF (3%)</th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
              ...
              pagination.paginatedItems.map((row, index) => (
                <tr
                  key={`${row.monthName}-${row.year}-${index}`}
                  className="border-t border-gray-100 hover:bg-blue-50 transition-colors duration-150"
                >
                  <td className="px-6 py-4 font-medium text-gray-900">
                    {row.monthName} {row.year}
                  </td>
                  <td className="px-6 py-4 text-right">{formatPaysheetMoney(row.epf_payment)}</td>
                  <td className="px-6 py-4 text-right">
                    {formatPaysheetMoney(row.employer_epf_payment)}
                  </td>
                  <td className="px-6 py-4 text-right">{formatPaysheetMoney(row.etf_payment)}</td>
                </tr>
              ))
            )}
          </tbody>
          {rows.length > 0 && (
            <tfoot>
              <tr className="bg-gray-100 font-bold text-gray-900 border-t-2 border-gray-200">
                ...
                <td className="px-6 py-4 text-right">{formatPaysheetMoney(totals.epf_payment)}</td>
                <td className="px-6 py-4 text-right">
                  {formatPaysheetMoney(totals.employer_epf_payment)}
                </td>
                <td className="px-6 py-4 text-right">{formatPaysheetMoney(totals.etf_payment)}</td>
              </tr>
            </tfoot>
          )}
        </table>
...
export default ContributionPeriodReportTable;
